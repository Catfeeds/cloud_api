...
class Store extends MY_Controller
{
    ...
    public function listStore()
    {
        ...
        $cities = Storemodel::where(['company_id' => $this->company_id])->groupBy('city')->get(['city']);
        $types = isset($post['city']) ? Storemodel::where(['company_id' => $this->company_id])
...
//        $store->describe    = strip_tags(htmlspecialchars_decode($store->describe));
    ...
    public function addStoreDot()
    {
        ...
        $images = $this->splitAliossUrl($post['images'], true);
        $images = json_encode($images);
        if (Storemodel::where(['company_id' => $this->company_id, 'rent_type' => 'DOT', 'name' => $post['name']])->first()) {
            $this->api_res(1008);
            return;
        }
        try {
            ...
            $insert->company_id = $this->company_id;
    ...
    public function addStore()
    {
        ...
        $images = $this->splitAliossUrl($post['images'], true);
        $images = json_encode($images);
        if (Storemodel::where(['company_id' => $this->company_id, 'rent_type' => $post['rent_type'], 'name' => $post['name']])->first()) {
            $this->api_res(1008);
            return;
        }
        try {
