...
class Employee extends MY_Controller {
    ...
    public function listEmp() {
        ...
        //define('$this->company_id', 4); //测试用
        $where = isset($post['store_id']) ? ['store_id' => $post['store_id']] : [];
        if (isset($post['city']) && !empty($post['city'])) {
            $this->load->model('storemodel');
            $store_ids = Storemodel::where('company_id', $this->company_id)
            ...
            $count = ceil((Employeemodel::where('company_id', $this->company_id)->count()) / PAGINATE);
            if ($page > $count) {
                $this->api_res(0, ['count' => $count, 'list' => []]);
                return;
            }
            $employees = Employeemodel::with(['position' => function ($query) {
                $query->select('id', 'name');
            }])->where('company_id', $this->company_id)->offset($offset)
    ...
    public function searchEmp() {
        ...
        $this->load->model('storemodel');
        $store_ids = Storemodel::where('company_id', $this->company_id)->get(['id'])->map(function ($s) {
            return $s->id;
        });
    ...
    public function submitEmp() {
        ...
        $name        = $post['name'];
        $isNameEqual = Employeemodel::where('company_id', $this->company_id)->where('name', $name)->first();
        if ($isNameEqual) {
            $this->api_res(1013);
            return false;
        }
        $phone        = $post['phone'];
        $isPhoneEqual = Employeemodel::where('company_id', $this->company_id)->where('phone', $phone)->first();
        if ($isPhoneEqual) {
            $this->api_res(1015);
            return false;
        }
        ...
        $this->load->model('positionmodel');
        $position = Positionmodel::where('company_id', $this->company_id)
