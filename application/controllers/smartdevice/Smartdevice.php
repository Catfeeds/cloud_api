...
class Smartdevice extends MY_Controller
{
	...
	public function listsmartdevice()
	{
		...
		if (!empty($post['device_type'])) {
			$where['type'] = trim($post['device_type']);
		}
	...
	public function fillDevice()
	{
		...
		$this->load->model('storemodel');
		$this->load->model('roomunionmodel');
		$device = new Smartdevicemodel();
		$res    = $device->addDevice($post);
		if ($res) {
			$this->api_res(0);
		} else {
			$this->api_res(1009);
	...
	public function validation()
	{
		...
		$config = [
			...
			[
				'field'  => 'type',
				'label'  => '设备类型',
				'rules'  => 'trim|required|in_list[COLD_WATER_METER,HOT_WATER_METER,ELECTRIC_METER,GAS_METER]',
				'errors' => [
					'required' => '请填写%s',
				],
			],
